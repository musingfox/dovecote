...
type AnyResult = { success: boolean; data?: { grantId: string }; error?: string };
...
test("revoke schema: valid 20-char alphanumeric grantId", () => {
  ...
  expect(asAny(result).data).toEqual({ grantId: "abcdefghijklmnopqrst" });
...
test("revoke schema: valid mixed-case grantId with underscore", () => {
  ...
  expect(result.success).toBe(true);
  expect(asAny(result).data).toEqual({ grantId: "5BJ6fIn2hfVOEiX8_test" });
  expect(asAny(result).error).toBeUndefined();
});
