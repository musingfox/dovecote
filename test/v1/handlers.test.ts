...
// ============================================================
// C2.4.c — GET /v1/env/:profile
// ============================================================

test("C2.4.c - GET /v1/env/:profile success with scope and valid profile", async () => {
  const { app } = buildApp(
    ENV_AUTH,
    makeServices({ readEnv: async () => "production=value" })
  );
  const env = buildTestEnv();

  const req = new Request("http://localhost/v1/env/dev");
  const res = await app.fetch(req, env, createMockExecutionContext());
  expect(res.status).toBe(200);

  const body = (await res.json()) as any;
  expect(body.profile).toBe("dev");
  expect(body.value).toBe("production=value");
});

test("C2.4.c - GET /v1/env/:profile 404 not_found (NotFoundError)", async () => {
  const { app } = buildApp(
    ENV_AUTH,
    makeServices({
      readEnv: async () => {
        throw new NotFoundError('Profile "dev" not found');
      },
    })
  );
  const env = buildTestEnv();
  ...
  const body = (await res.json()) as any;
  expect(body.error).toBe("not_found");
  expect(body.error_description).toBe('Profile "dev" not found');
});

test("C2.4.c - GET /v1/env/:profile 400 invalid_request (bad profile name regex)", async () => {
  const { app } = buildApp(ENV_AUTH);
  const env = buildTestEnv();

  // "dev!" violates /^[a-zA-Z0-9_-]+$/
  const req = new Request("http://localhost/v1/env/dev!");
  const res = await app.fetch(req, env, createMockExecutionContext());
  expect(res.status).toBe(400);
  ...
  const body = (await res.json()) as any;
  expect(body.error).toBe("invalid_request");
  expect(body.error_description).toBe("Profile must match /^[a-zA-Z0-9_-]+$/");
});

test("C2.4.c - GET /v1/env/:profile 403 forbidden (missing scope)", async () => {
  const { app } = buildApp(
    NO_SCOPE_AUTH,
    makeServices({
      readEnv: async () => {
...
test("C2.4.c - GET /v1/env/:profile 502 upstream_error (UpstreamError)", async () => {
  const { app } = buildApp(
    ENV_AUTH,
    makeServices({
      readEnv: async () => {
        throw new UpstreamError("KV read failed");
      },
    })
  );
  const env = buildTestEnv();
  ...
  const body = (await res.json()) as any;
  expect(body.error).toBe("upstream_error");
  expect(body.error_description).toBe("KV read failed");
});

test("C2.4.c - GET /v1/env/:profile success with empty string value", async () => {
  const { app } = buildApp(
    ENV_AUTH,
    makeServices({ readEnv: async () => "" })
  );
  const env = buildTestEnv();

  const req = new Request("http://localhost/v1/env/empty");
  const res = await app.fetch(req, env, createMockExecutionContext());
  expect(res.status).toBe(200);

  const body = (await res.json()) as any;
  expect(body.profile).toBe("empty");
