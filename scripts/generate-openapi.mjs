...
// OpenApiSurfaceSync: /v1/auth/whoami (new surface after trimming device+general-oidc).
// Uses 401/500 only (no admin gate, like exchange carve-outs).
...
registry.registerPath({
  method: "get",
  path: "/v1/auth/whoami",
  description: "Return the dvct_* bearer identity (userId, scopes, tokenId, expiry).",
  responses: {
    200: jsonOk(whoamiResponseSchema, "Identity returned"),
    401: errorResponse("Missing or invalid Authorization header"),
    500: errorResponse("Internal server error"),
  },
});
